...
namespace CameraServicesPlatform.BackEnd.API.Controllers
{
    ...
    public class WishlistController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetWishlistByMemberID(string memberID, int pageIndex = 1, int pageSize = 10)
        {
            try
            {
                var response = await _wishlistService.GetWishlistByMemberID(memberID, pageIndex, pageSize);
                if (!response.IsSuccess)
                {
                    return NotFound(response);
                }
