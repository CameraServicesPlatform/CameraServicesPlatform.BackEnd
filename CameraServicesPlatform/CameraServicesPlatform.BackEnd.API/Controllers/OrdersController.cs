...
namespace CameraServicesPlatform.BackEnd.API.Controllers
{
    ...
    public class OrdersController : ControllerBase
    {
        ...
        public async Task<AppActionResult> GetOrderByOfSupplier(Guid SupplierId, int pageIndex = 1, int pageSize = 10)
        {
            return await _orderService.GetOrderOfSupplier(SupplierId, pageIndex, pageSize);
        }

        [HttpGet("get-order-of-member")]
        public async Task<AppActionResult> GetOrderByMemberId(Guid MemberId, int pageIndex = 1, int pageSize = 10)
        {
